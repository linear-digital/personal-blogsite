...
export default async function HeroPost5() {
    const posts = await fetcher({ path: "/blog?limit=5&skip=4" })
    return (
        <>
                            ...
                            {/* <div className="post-card top-post top-2 mb-25">
                                <div className="post-thumb">
                                    <Link href={postLink(posts[1])}>
                                        <img src={postImage(posts[1])} alt={posts[1].title} />
                                    </Link>
                                </div>
                                <div className="post-content">
                                    ...
                                    <h3 className="title">
                                        <Link href="/single-post-1">{posts[1]?.title?.slice(0, 60)}</Link>
                                    </h3>
                                    <ul className="post-list">
                                        ...
                                        <li><i className="las la-comments" />0</li>
                                    </ul>
                                </div>
                            </div> */}
                            <div className="post-card gadget-card mb-25">
                                <div className="post-thumb">
                                    <Link href={postLink(posts[1])}><img src={postImage(posts[1])} alt="post" /></Link>
                                </div>
                                <div className="post-content">
                                    {posts[1]?.tags?.slice(0, 2).map((tag, i) => (
                                        <Link
